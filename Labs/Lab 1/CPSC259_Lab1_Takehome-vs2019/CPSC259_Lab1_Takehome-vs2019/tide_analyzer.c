...
int main(void)
{
	...
	double complex_component[NUMBER_OF_READINGS] = { 0 }; /* Can be used for the complex results of fft */
	...
	*/
	...
	fft(1, M, readings, complex_component);
...
void process_file(double array_to_populate[], FILE* pointer_to_data_file)
{
	...
	while(fgets(line_buffer, LINESIZE, pointer_to_data_file)) {
		...
		for (i = 0; i < values_per_line; i++, readings_processed++) {
